...
def _visible_appointment(db: Session, item_id: uuid.UUID, user: User) -> Appointment:
    item = db.query(Appointment).filter(Appointment.id == item_id, Appointment.clinic_id == user.clinic_id).first()
    if item is None:
...
def detail(item_id: uuid.UUID, request: Request, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> Appointment:
    item = _visible_appointment(db, item_id, user)
    record_audit_event(action=AuditAction.STAFF_VIEWED_APPOINTMENT if user.role != UserRole.PATIENT else AuditAction.PATIENT_VIEWED_OWN_RECORD, result=AuditResult.SUCCESS, clinic_id=user.clinic_id, actor_user_id=user.id, actor_email=user.email, resource_type="appointment", resource_id=item.id, ip_address=client_ip(request))
    return item


@router.patch("/{item_id}", response_model=AppointmentPublic)
def update(item_id: uuid.UUID, payload: AppointmentUpdateRequest, request: Request, db: Session = Depends(get_db), user: User = Depends(_staff_or_admin_only)) -> Appointment:
    item = _visible_appointment(db, item_id, user)
    ...
    db.commit()
    db.refresh(item)
    record_audit_event(action=AuditAction.APPOINTMENT_UPDATED, result=AuditResult.SUCCESS, clinic_id=user.clinic_id, actor_user_id=user.id, actor_email=user.email, resource_type="appointment", resource_id=item.id, ip_address=client_ip(request))
    return item


@router.post("/{item_id}/cancel", response_model=AppointmentPublic)
def cancel(item_id: uuid.UUID, request: Request, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> Appointment:
    item = _visible_appointment(db, item_id, user)
    if item.status in (AppointmentStatus.CANCELLED, AppointmentStatus.COMPLETED):
        raise HTTPException(status_code=409, detail="Consulta encerrada.")
    ...
    db.commit()
    db.refresh(item)
    record_audit_event(action=AuditAction.APPOINTMENT_CANCELLED, result=AuditResult.SUCCESS, clinic_id=user.clinic_id, actor_user_id=user.id, actor_email=user.email, resource_type="appointment", resource_id=item.id, ip_address=client_ip(request))
    return item
